...
def machine(monkeypatch, tmp_path):
    ...
    m = {"smi": "NVIDIA GeForce RTX 4070, 12282 MiB, 581.15\n", "free_gb": 200, "down": set(), "token_ok": True,
         "cuda": True, "login_ok": True, "tokens": [], "logins": []}
    ...
    def sign_in(email, password):
        m["logins"].append((email, password))
        if not m["login_ok"]:
    ...
    monkeypatch.setattr(sb, "sign_in", sign_in)
...
def run_preflight(answers: list[str]) -> tuple[int, str]:
    out = []
    it = iter(answers)
    code = preflight.run(ask=lambda prompt: next(it), secret=lambda prompt: next(it), say=out.append)
    return code, "\n".join(out)


def test_healthy_machine_passes_and_stores_token_and_session(machine, tmp_path):
    code, out = run_preflight(["hf_abc", "tech@dept.gov", "pw"])
    ...
    assert machine["tokens"] == ["hf_abc"] and machine["logins"] == [("tech@dept.gov", "pw")]
...
def test_rejected_token_asks_again_then_gives_up_with_a_fix(machine):
    ...
    code, out = run_preflight(["hf_bad", "hf_bad2", "hf_bad3", "tech@dept.gov", "pw"])
...
def test_wrong_password_asks_again_and_succeeds(machine):
    machine["login_ok"] = False
    calls = {"n": 0}
    real = sb.sign_in
    ...
    def flaky(email, password):
        calls["n"] += 1
        if calls["n"] == 2:
            machine["login_ok"] = True
        return real(email, password)

    sb.sign_in = flaky
    code, out = run_preflight(["hf_abc", "tech@dept.gov", "typo", "tech@dept.gov", "pw"])
    assert code == 0 and "Invalid login credentials" in out and store.session()["email"] == "tech@dept.gov"


def test_empty_token_skips_weights_with_a_warning(machine, tmp_path):
    code, out = run_preflight(["", "tech@dept.gov", "pw"])
